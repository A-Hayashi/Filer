        ...
        public override void execute()
        {
            ...
            if(main_form.FocusedViewType == Form1.EFocusedViewType.FileView)
            {
                ...
                foreach(FilerItem item in list)
                {
                    ...
                    {
                        Microsoft.VisualBasic.FileIO.FileSystem.CopyFile(item.Info.FullName, to_folder + "\\" + item.Info.Name, Microsoft.VisualBasic.FileIO.UIOption.OnlyErrorDialogs);
                    }
                    else
                    {
                        Microsoft.VisualBasic.FileIO.FileSystem.CopyDirectory(item.Info.FullName, to_folder, Microsoft.VisualBasic.FileIO.UIOption.AllDialogs);
